...
import {
  addNewDonorPayment,
} from "store/donorcheckout/actions";
...
class DonorPayment extends Component {
  ...
  handleProceedClick = () => {
    ...
    if (canProceed) {
      ...
      const { onAddDonorPaymentData } = this.props;
      setTimeout(() => {
        console.log(
          onAddDonorPaymentData(this.state.donorPayment, this.state.user_id)
        );
      }, 2000);

      setTimeout(() => {
        this.setState({ donorPayment: this.props.donorPayment });

        // If checkout operation is successful.
        if (this.props.donorPayment) {
  ...
  // // API call to get the checkout items
  // const { onGetDonorPaymentItems } = this.props;
  // setTimeout(() => {
  // onGetDonorPaymentItems(this.state.user_id, "Yes");
  // }, 1000);

  // setTimeout(() => {
  // this.setState({ donorPayments: this.props.donorPayments });
  // }, 2000);
  // }
  // componentDidMount() {
  //   // API call to get the checkout items
  ...
  componentDidUpdate(prevProps, prevState, snapshot) {
    ...
    const { donorPayments } = this.props;
    if (
      isEmpty(prevProps.donorPayments) &&
      !isEmpty(donorPayments) &&
      size(donorPayments) !== size(prevProps.donorPayments)
    ) {
      this.setState({ donorPayments });
    }
    ...
    return (
      <React.Fragment>
        <div className="page-content">
          <MetaTags>
            <title>DonorPayment | Lab Hazir - Dashboard</title>
          </MetaTags>
          <Container fluid>
            {/* Render Breadcrumb */}
            <Breadcrumbs title="Test" breadcrumbItem="DonorPayment" />
            <Formik>
...
<p className="font-weight-bold mb-4">Donor Info</p>
</NavLink>
</NavItem> */}
...
DonorPayment.propTypes = {
  ...
  history: any,
  donorPayments: PropTypes.array,
  // onGetDonorPaymentItems: PropTypes.func,
  onAddDonorPaymentData: PropTypes.func,
  donorPayment: PropTypes.array,
};

// const mapStateToProps = state => ({
//   donorPayments: state.donorcheckout.donorPayments,
//   checkedoutData: state.donorcheckout.checkedoutData,
// });
const mapStateToProps = ({ donorPayments }) => ({
  donorPayments: donorPayments.donorPayments,
  donorPayment: donorPayments.donorPayment,

});

const mapDispatchToProps = (dispatch, ownProps) => ({

  onAddDonorPaymentData: (donorPayment, id) =>
    dispatch(addNewDonorPayment(donorPayment, id)),
});
