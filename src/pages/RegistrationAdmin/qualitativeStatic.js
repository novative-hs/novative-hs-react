...
class Results extends Component {
    ...
    this.state = {
      ...
      columnsList: [
       ...
       
        {
          ...
          headerAlign: "center",
          formatter: (cellContent, analyte) => (
            <div className="text-center">{analyte.correct_result}</div>
          ),
        },
        {
          ...
          headerAlign: "center",
          formatter: (cellContent, analyte) => (
            <div className="text-center">{analyte.wrong_result}</div>
          ),
        },
        {
          text: "Not Submitted",
          ...
          headerAlign: "center",
          formatter: (cellContent, analyte) => (
            <div className="text-center">{analyte.not_submitted}</div>
          ),
        },
      ],
    };
  ...
  combineData = (analytes, results) => {
    return analytes.map(analyte => {
      ...
      console.log("MATCHED ENTRY FOR:", analyte.name, resultEntry); // ✅ add this
      return {
        ...analyte,
        lab_count: resultEntry?.lab_count || 0,
        true_value: resultEntry?.true_value || 0,
        correct_result: resultEntry?.correct_result || 0,
        wrong_result: resultEntry?.wrong_result || 0,
        not_submitted: resultEntry?.not_submitted || 0,
      };
    });
  };
  ...
  componentDidMount() {
    ...
    // Only fetch data, no calculation
    Promise.all([onGetSchemeAnalyte(id), onGetStatisticsList(id)]).catch(
      error => {
  ...
  componentDidUpdate(prevProps) {
    ...
    if (
      ...
      const combinedData = this.combineData(
        SchemeAnalytesList,
        Statistics // ✅ use Statistics here
      );
              ...
              <Col>
                <Row className="mb-3" style={{ marginLeft: "80px" }}>
                  ...
                  <Col md={2}>
                    <Col md={12}>
                      <Tooltip
                        title="Please assign values before calculating"
                        placement="top"
                      >
                        <span>
                          <Button
