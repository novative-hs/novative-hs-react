      ...
      pendingLabListColumns: [
        {
          ...
          formatter: (cellContent, lab) => (
            <span style={{ display: "flex", justifyContent: "flex-start", gap: "10px" }}>
              <Link to="#" onMouseEnter={e => this.openLabModal(e, lab)} onPointerLeave={this.handleMouseExit}>
                {lab.province
}
              </Link>
            </span>
        ...
        },
        {
          ...
          filter: textFilter(),
          formatter: (cellContent, lab) => <span style={{ display: "flex", justifyContent: "center", gap: "10px" }}>{lab.district}</span>,
        },
        {
          dataField: "city",
          text: "City",
          ...
          style: { textAlign: "center" },
          filter: textFilter(),
          formatter: (cellContent, lab) => <span style={{ display: "flex", justifyContent: "center", gap: "10px" }}>{lab.city}</span>,
        },
        {
          ...
          style: { textAlign: "center" },
          filter: textFilter(),
          formatter: (cellContent, lab) => <span style={{ display: "flex", justifyContent: "center", gap: "10px" }}>{lab.total_labs}</span>,
        },
        {
          ...
          filter: textFilter(),
          formatter: (cellContent, lab) => (
            <span style={{ display: "flex", justifyContent: "center", width: "200px", gap: "30px", wordBreak: "break-word", overflowWrap: "break-word" }}>
        ...
        
        {
          ...
          style: { textAlign: "center" },
          filter: textFilter(),
          formatter: (cellContent, lab) => <span style={{ display: "flex", justifyContent: "center", gap: "10px" }}>{lab.active_labs}</span>,
        },
        {
          ...
          style: { textAlign: "center" },
          filter: textFilter(),
          formatter: (cellContent, lab) => <span style={{ display: "flex", justifyContent: "center", gap: "10px" }}>{lab.inactive_labs}</span>,
        },
         {
          ...
          style: { textAlign: "center" },
          filter: textFilter(),
          formatter: (cellContent, lab) => <span style={{ display: "flex", justifyContent: "center", gap: "10px" }}>{lab.suspended_labs}</span>,
        },
        // {
...
aggregateLabs(rawLabs) {
  ...
  rawLabs.forEach(lab => {
    ...
    else if (normalizedStatus === "suspended") grouped[key].suspended_labs += 1;
    else if (normalizedStatus === "new_registered") grouped[key].new_register_labs += 1;
  });
...
componentDidMount() {
  ...
  if (prevProps.AllLabs !== this.props.AllLabs) {
    ...
    }, this.applyFilters); // If you have extra filtering logic, else remove this line
  }

  if (prevProps.CycleList !== this.props.CycleList || prevProps.AllLabs !== this.props.AllLabs) {
    const { CycleList, AllLabs } = this.props;
    const participantSchemes = new Set(
      (AllLabs || []).flatMap(lab => lab.schemes?.map(s => s.scheme_name) || [])
    );
    const filteredCycleList = (CycleList || []).filter(cycle => participantSchemes.has(cycle.scheme_name));
  ...
  fetchData(user_id) {
    const { onGetParticipantPayment, ongetApprovedLabs, ongetcyclelist, onGetPendingLabs } = this.props;
  ...
  applyFilters() {
  const { filteredLabs: currentAggregatedLabs, selectedParticipantType, selectedScheme } = this.state;
  ...
  toggleUnapprovedModal() {
    this.setState(prevState => ({ unapprovedModal: !prevState.unapprovedModal }));
  }
...
const mapStateToProps = (state) => {
  ...
  console.log("ProfileMenu mapStateToProps - participantCounts:", participantCounts); // ✅ log this too
...
export default connect(mapStateToProps, mapDispatchToProps)(Membershipstatusreport);