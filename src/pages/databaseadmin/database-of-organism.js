...
class OrganinismList extends Component {
  constructor(props) {
    super(props);
    ...
    this.state = {
      ...
      ReagentsListColumns: [
...

        {
          ...
          editable: false,
          text: "Action",
          formatter: (cellContent, organism) => (
            <div className="d-flex gap-3 ml-3" style={{ display: 'flex', justifyContent: 'center', gap: '10px' }}>
              <Tooltip title="Update">
                <Link className="text-success" to="#">
                  <i
                    ...
                    id="edittooltip"
                    onClick={e => this.handleReagentsClick(e, organism)}
                  ></i>
                </Link>
              </Tooltip>
              ...
              {/* <Tooltip title="History">
                <Link
                  ...
                  to={`/${this.state.organization_name}/databaseadmin-history/${organism.id}?type=organism`}
                  onClick={e => {
                    e.preventDefault();
                    ...
                    const url = `/${this.state.organization_name}/databaseadmin-history/${organism.id}?type=organism`;
              ...
              <Tooltip title="Delete">
                <Link className="text-danger" to="#">
                  <i
                    ...
                    id="deletetooltip"
                    onClick={() => this.onClickDelete(organism)}
                  ></i>
                </Link>
              </Tooltip>
            </div>
  ...
  componentDidMount() {
    ...
    const { ListOrganism, onGetOrganism } = this.props;
    ...
    if (userId) {
      ...
      onGetOrganism(userId); // Fetch organism list
    } else {
      // console.error("User ID is undefined or null.");
    }

    // Set ListOrganism in state with a fallback
    this.setState({
      ListOrganism: ListOrganism || [],
...
onClickDelete = organism => {
  this.setState({ ListOrganism: organism, deleteModal: true });
};


handleDeleteInstrumentType = () => {
  const { onDeleteOrganism } = this.props;
  const { ListOrganism } = this.state;
  
  if (ListOrganism.id !== undefined) {
    onDeleteOrganism(ListOrganism);
    setTimeout(() => {
      this.props.onGetOrganism(this.state.user_id);
    }, 1000);
  ...
  handleReagentsClicks = () => {
    this.setState({ organism: "", isEdit: false });
    this.toggle();
  };

  componentDidUpdate(prevProps) {
    const { ListOrganism} = this.props;

    // Check if `ListOrganism` or `SchemeList` has changed
    if (prevProps.ListOrganism !== ListOrganism ) {
      console.log("Updating filtered schemes based on ListOrganism ");

      // Extract unique scheme names from `ListOrganism`
      // const organismSchemes = new Set(
      //   ListOrganism.flatMap(entry => entry.schemes || []) // `schemes` is assumed to be an array of scheme names
      // );

      // // Filter `SchemeList` to include only schemes present in `ListOrganism`
  ...
  exportToExcel = () => {
    ...
    const dataToExport = ListOrganism.map(unit => ({
      id: unit.id,
      name: unit.name,
      code: unit.code,
      status: unit.status,
    ...
    try {
      ...
      reader.onload = async e => {
        ...
        for (let i = 0; i < jsonData.length; i++) {
          ...
          await this.props.onAddNewOrganism({
            name: item.name,
            code: item.code,
            status: item.status,
        ...
        await this.props.onGetOrganism(this.state.user_id);
      };
  ...
  render() {
    ...
    const filteredData = ListOrganism.filter(entry => {
    ...
    const { onAddNewOrganism, onUpdateorganism, onGetOrganism } = this.props;
    ...
    return (
      <React.Fragment>
        ...
        <div className="page-content">
          <MetaTags>
            <title>Organism List | NEQAS</title>
          </MetaTags>
          <Container fluid>
            {/* Render Breadcrumbs */}
            <Breadcrumbs title="Organism" breadcrumbItem="Organism List" />
                  ...
                  <button
                    ...
                    onClick={e => {
                      ...
                      const downloadUrl =
                        ...
                        "/media/public/organism.xlsx";
                      saveAs(downloadUrl, "organism.xlsx"); // Use the file-saver library to trigger the download
                    }}
                  >
                          ...
                          {toolkitprops => (
                            <React.Fragment>
                              <Row className="mb-2">
                                ...
                                <Col xl="4">
                                  <div className="form-group">
                                    <label htmlFor="statusFilter">Filter by organism Status</label>
                                    <select
                                      id="statusFilter"
                                      value={this.state.statusFilter}
                                      onChange={e => this.handleFilterChange("statusFilter", e)}
...
OrganinismList.propTypes = {
  match: PropTypes.object,
  ListOrganism: PropTypes.array,
  onDeleteOrganism: PropTypes.func,
  className: PropTypes.any,
  createInstrumentType: PropTypes.array,
  onGetOrganism: PropTypes.func,
  onAddNewOrganism: PropTypes.func,
  onUpdateorganism: PropTypes.func,
...
const mapStateToProps = (state) => {
  const ListOrganism = state.ListOrganism?.ListOrganism || [];
  console.log("Extracted ListOrganism:", ListOrganism);
  return {
    ListOrganism,
  };
};
...
const mapDispatchToProps = (dispatch, ownProps) => ({
  onGetOrganism: id => dispatch(getOrganismlist(id)),
 onAddNewOrganism: (createorganism, id) =>
  dispatch(addOrganismlist(createorganism)),
 onUpdateorganism: organism => dispatch(updateorganism(organism)),
 onDeleteOrganism: organism => dispatch(deleteorganism(organism)),
});

export default connect(
  mapStateToProps,
  mapDispatchToProps
)(withRouter(OrganinismList));