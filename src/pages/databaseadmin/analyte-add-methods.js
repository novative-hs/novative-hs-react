...
class AnalyteAddMethods extends Component {
  ...
  componentDidMount() {
    const { ListMethods, onGetMethodList } = this.props;
    onGetMethodList(this.state.user_id);
  ...
  handleSelectChange = (e, id) => {
    const updatedList = this.state.ListMethods.map(item =>
      item.id === id ? { ...item, status: e.target.value } : item
    );
    this.setState({ ListMethods: updatedList });
  };
  ...
  render() {
    ...
    const { ListMethods } = this.props;
    const methodlist = this.state.ListMethods;
