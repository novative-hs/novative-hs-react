        ...
        <>
          <div>
            <input
              ...
              onChange={e => this.handleFilterChange('cycleNumberFilter', e)}
              className="form-control"
              placeholder=""
            />
          </div>
...
,
    ...
    <div className="d-flex justify-content-center align-items-center">
      {receiptUrl ? (
        ...
        <span className="text-center">
          {paymentMethod} <span className="text-muted">(No Receipt)</span>
        </span>
      )}
    </div>
  );
      ...
      <div style={{ textAlign: "center" }}>
        <div style={{ marginBottom: "5px" }}>
          <input
            ...
            onChange={(e) => this.handleFilterChange("paymentModeFilter", e)}
            className="form-control"
            style={{
              textAlign: "center",
              width: "100px",
 ...
 componentDidUpdate(prevProps) {
  ...
  filterData = () => {
  const { PaymentSchemeList, nameFilter, idFilter, cycleNumberFilter, priceFilter, discountFilter, paidAmountFilter, payDateFilter, paymentModeFilter, receivedByFilter , photoUrlFilter } = this.state;
  ...
  return (
    ...
    (entry.payment_mode || "").toLowerCase().includes((paymentModeFilter || "").toLowerCase()) &&
    (entry.received_by || "").toLowerCase().includes((receivedByFilter || "").toLowerCase()) &&
    (!photoUrlFilter || (entry.photo_url || "").toLowerCase().includes(photoUrlFilter.toLowerCase()))
    ...
    return (
      <React.Fragment>
        <div className="page-content">
          ...
          <Breadcrumbs
  title="List"
  breadcrumbItem={`${this.props.participantName || "Unknown"} (${this.props.membershipStatus || "Inactive"})`}
/>
...
const mapStateToProps = (state) => {
  ...
  console.log("Mapped PaymentSchemeList:", state.AddPayment?.PaymentSchemeList || []);
