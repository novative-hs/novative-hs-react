import {
  ...
  ADD_NEW_METHOD_ANALYTE_SUCCESS,
...
const ListUnit = (state = INIT_STATE, action) => {
  switch (action.type) {
    ...
    case GET_ANALYTEFORSCHEME_LIST_SUCCESS:
      console.log("Data received in success action:", action.payload); // Log the action payload
      console.log(
        "Reducer - Updated State with SchemeName:",
        action.payload.SchemeName
      );
      return {
        ...state,
    ...
    case GET_SCHEMEANALYTE_LIST_SUCCESS:
      ...
      return {
        ...state,
        SchemeAnalyteList: Array.isArray(action.payload.analytes)
          ? action.payload.analytes
          : [],
    ...
    case GET_REAGENT_ANALYTE_LIST_FAIL:
      console.error(
        "Error Fetching ReagentAnalyteList Analytes:",
        action.payload
      );
      return {
        ...state,
    ...
    case ADD_NEW_REAGENT_ANALYTE_SUCCESS:
      return {
        ...
        AddReagentAnalyte: [...state.AddReagentAnalyte, action.payload.data],
      };
    ...
    /////////////////////////////////////////////////////
    case GET_METHOD_ANALYTE_LIST_SUCCESS:
      console.log("✅ REagnt Analyte List:", action.payload.analytes);
      console.log("✅ reagnet Name:", action.payload.method_name);
      console.log("✅ Active Schemes Received:", action.payload.active_schemes);

      return {
        ...state,
        MethodAnalyteList: action.payload.analytes,
        methodName: action.payload.method_name || "Unknown Reagent",
        activeSchemes: action.payload.active_schemes || [],
      };

    case GET_METHOD_ANALYTE_LIST_FAIL:
      console.error(
        "Error Fetching MethodAnalyteList Analytes:",
        action.payload
      );
      return {
        ...state,
    ...
    case ADD_NEW_METHOD_ANALYTE_SUCCESS:
      return {
        ...state,
        MethodAnalyteList: [
          ...state.MethodAnalyteList,
          ...(Array.isArray(action.payload.data)
            ? action.payload.data
            : [action.payload.data]),
        ],
        AddMethodAnalyte: [...state.AddMethodAnalyte, action.payload.data],
      };

    case ADD_NEW_METHOD_ANALYTE_FAIL:
      return {
        ...state,
        error: action.payload,
      };
    case UPDATE_METHOD_ANALYTE_SUCCESS:
      return {
        ...state,
        MethodAnalyteList: state.MethodAnalyteList.map((schemeanalyte) =>
          schemeanalyte.id.toString() === action.payload.id.toString()
            ? { schemeanalyte, ...action.payload }
            : schemeanalyte
        ),
    ...
    case UPDATE_METHOD_ANALYTE_FAIL:
      return {
        ...state,
        error: action.payload,
      };
